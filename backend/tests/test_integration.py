...
class TestRealVectorStoreIntegration:
    ...
    def test_vector_store_with_sample_data(self, temp_chroma_path, sample_course, sample_course_chunks):
        ...
        found_ml_content = any("machine learning" in doc.lower() for doc in results.documents)
    ...
    def test_vector_store_zero_max_results_bug(self, temp_chroma_path, sample_course, sample_course_chunks):
        ...
        print(f"⚠️  With MAX_RESULTS=0, search returned {len(results.documents)} documents")
        ...
        print("🐛 CONFIRMED: MAX_RESULTS=0 causes searches to return no results despite having data")
    
    def test_vector_store_with_fixed_max_results(self, temp_chroma_path, sample_course, sample_course_chunks):
        ...
        print(f"✓ With MAX_RESULTS=5, search returned {len(results.documents)} documents")
        assert len(results.documents) > 0, "Should return results with proper MAX_RESULTS"
...
class TestRAGSystemIntegration:
    ...
    def test_rag_system_with_real_vector_store(self, mock_ai_gen, temp_chroma_path, sample_course, sample_course_chunks):
        """Test RAG system with real vector store but mocked AI"""
        
        # Mock AI generator to avoid actual API calls
        mock_ai_instance = Mock()
        mock_ai_instance.generate_response.return_value = "Mocked AI response about machine learning"
    ...
    @patch('rag_system.AIGenerator')
    def test_search_tool_with_real_vector_store(self, mock_ai_gen, temp_chroma_path, sample_course, sample_course_chunks):
...
class TestEndToEndScenarios:
    ...
    def test_complete_query_flow_simulation(self, mock_anthropic, temp_chroma_path, sample_course, sample_course_chunks):
        ...
        mock_final_response.content = [Mock(text="Machine learning is a subset of AI that focuses on algorithms, as explained in the Introduction to Machine Learning course.")]
        ...
        assert response == "Machine learning is a subset of AI that focuses on algorithms, as explained in the Introduction to Machine Learning course."
    ...
    def test_error_propagation_simulation(self, temp_chroma_path):
        ...
        print("✓ Error propagation tested - system fails gracefully with invalid API key")
...
class TestSystemHealthChecks:
    ...
    def test_system_health_with_current_config(self):
        ...
        print(f"MAX_RESULTS: {config.MAX_RESULTS} {'❌ BUG!' if config.MAX_RESULTS == 0 else '✅'}")
        ...
        if chroma_accessible:
            try:
                ...
                print(f"Course Count: {course_count} {'✅' if course_count > 0 else '⚠️  No data'}")
