...
class TestConfigurationIssues:
    ...
    def test_max_results_is_zero_bug(self):
        ...
        """
        assert config.MAX_RESULTS == 0, f"Expected MAX_RESULTS=0 bug, got {config.MAX_RESULTS}"
    ...
    def test_anthropic_model_configured(self):
        ...
        assert config.ANTHROPIC_MODEL == expected_model, f"Expected {expected_model}, got {config.ANTHROPIC_MODEL}"
    ...
    def test_embedding_model_configured(self):
        ...
        assert config.EMBEDDING_MODEL == expected_model, f"Expected {expected_model}, got {config.EMBEDDING_MODEL}"
        print(f"✓ Embedding model: {config.EMBEDDING_MODEL}")
    
    def test_chunk_settings_reasonable(self):
        """Test that chunk size and overlap are reasonable"""
        assert config.CHUNK_SIZE > 0, f"CHUNK_SIZE should be positive, got {config.CHUNK_SIZE}"
        assert config.CHUNK_OVERLAP >= 0, f"CHUNK_OVERLAP should be non-negative, got {config.CHUNK_OVERLAP}"
    ...
    def test_max_history_reasonable(self):
        ...
        assert config.MAX_HISTORY >= 0, f"MAX_HISTORY should be non-negative, got {config.MAX_HISTORY}"
...
class TestChromaDBInitialization:
    ...
    def test_chromadb_with_zero_max_results(self, temp_chroma_path):
        ...
        print(f"⚠️  Search with MAX_RESULTS=0 returned: {len(results.documents)} documents")
...
class TestDocumentLoading:
    ...
    def test_docs_folder_exists(self):
        ...
        print(f"📁 Checking docs paths:")
        print(f"   Relative: {docs_path} -> exists: {os.path.exists(docs_path)}")
        print(f"   Absolute: {absolute_docs_path} -> exists: {os.path.exists(absolute_docs_path)}")
        ...
        actual_docs_path = docs_path if os.path.exists(docs_path) else absolute_docs_path
        
        # Check for document files
        doc_files = [f for f in os.listdir(actual_docs_path) 
...
class TestSystemInitialization:
    ...
    def test_tool_manager_setup(self, test_config, temp_chroma_path):
        ...
        search_tool_found = any(tool['name'] == 'search_course_content' for tool in tool_definitions)
