...
class TestRAGSystemInitialization:
    ...
    def test_rag_system_initialization(self, mock_session_mgr, mock_doc_proc, mock_ai_gen, mock_vector_store, test_config):
    ...
    def test_tool_manager_setup(self, test_config, temp_chroma_path):
        ...
        with patch('rag_system.VectorStore') as mock_vs, \
            ...
            assert len(tool_definitions) >= 2  # Should have at least search and outline tools
...
class TestRAGSystemQuery:
    ...
    def test_query_without_session(self, test_config):
        ...
        mocks['ai_generator'].generate_response.return_value = "AI response about machine learning"
        ...
        with patch.object(rag_system.tool_manager, 'get_last_sources') as mock_sources, \
            ...
            mock_sources.return_value = [{"text": "ML Course", "link": "https://example.com"}]
    ...
    def test_query_with_session(self, test_config):
        ...
        mocks['session_manager'].get_conversation_history.return_value = "Previous: Hello"
        ...
        with patch.object(rag_system.tool_manager, 'get_last_sources') as mock_sources, \
            ...
            response, sources = rag_system.query("Follow-up question", session_id="session123")
            
            # Verify session manager was used
            mocks['session_manager'].get_conversation_history.assert_called_once_with("session123")
    ...
    def test_query_exception_handling(self, test_config):
        ...
        mocks['ai_generator'].generate_response.side_effect = Exception("AI service failed")
...
class TestRAGSystemDocumentManagement:
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_add_course_document(self, mock_sm, mock_dp, mock_ai, mock_vs, test_config, sample_course, sample_course_chunks):
        ...
        mock_doc_processor.process_course_document.assert_called_once_with("/fake/path/course.pdf")
    ...
    @patch('os.listdir')
    def test_add_course_folder(self, mock_listdir, mock_exists, mock_sm, mock_dp, mock_ai, mock_vs, test_config):
        """Test adding courses from a folder"""
        
        # Set up mocks
        mock_exists.return_value = True
        mock_listdir.return_value = ['course1.pdf', 'course2.txt', 'course3.docx', 'readme.md']
        
        mock_vector_store = Mock()
        mock_vector_store.get_existing_course_titles.return_value = []  # No existing courses
        mock_vs.return_value = mock_vector_store
        ...
        # Add course folder
        total_courses, total_chunks = rag_system.add_course_folder("/fake/docs", clear_existing=False)
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_document_processing_error_handling(self, mock_sm, mock_dp, mock_ai, mock_vs, test_config):
...
class TestRAGSystemAnalytics:
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_get_course_analytics(self, mock_sm, mock_dp, mock_ai, mock_vs, test_config):
